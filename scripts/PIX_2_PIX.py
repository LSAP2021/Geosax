"""
@Armel Nya
This script are used to create your first model Neuronal Network
...
if gpus:
    try:
        ...
        print(e)
# PATH DATA
pathtrain = os.path.join("/home/armel/PAIRGEMM/scripts/TRAINING_DATA/")  # source directory training
IMG_TR = glob(pathtrain + "/*.jpeg")
pathtest = os.path.join("/home/armel/PAIRGEMM/scripts/TESTING_DATA/")  # source directory testing
IMG_TEST = glob(pathtest + "/*.jpeg")
...
def load(image_file):
    ...
    real_image, input_image = image, image
...
def create_dataset(path_to_train_images_record, path_to_test_images_record, buffer_size,
                   batch_size):
    """Creates a tf.data Dataset.
    Args:
      path_to_train_images_record: Path to train images folder.
      path_to_test_images_record: Path to test images folder.
      buffer_size: Shuffle buffer size.
      batch_size: Batch size
    Returns:
      train dataset, test dataset
    """
    train_dataset = tf.data.Dataset.list_files(path_to_train_images_record)
    train_dataset = train_dataset.shuffle(buffer_size)
    train_dataset = train_dataset.map(
        load_image_train, num_parallel_calls=AUTOTUNE)
    train_dataset = train_dataset.batch(batch_size)

    test_dataset = tf.data.Dataset.list_files(path_to_test_images_record)
...
def main(epochs, enable_function, path, buffer_size, batch_size):
    ...
    train_dataset, _ = create_dataset(
        os.path.join(path_to_folder, 'train_data/*.jpeg'),
        os.path.join(path_to_folder, 'test_data/*.jpeg'),
